...
class Bitrix {
  ...
  async getDataByPage(resource, total) {
    let query = {};
    const requests = []; // generate array of requests
    ...
    return Promise.all(requests).then(data => flattenDepth_1(data, 1));
  }

  async getDataByBatch(resource, total) {
    const requests = []; // generate a batch request for each 2500 entries
    ...
    for (let processed = 0; processed <= total; processed += MAX_ENTRIES_PER_BATCH) {
      ...
      let query = {}; // generate query parameters object for each request
    ...
    return Promise.all(requests).then(data => flattenDepth_1(data, 2));
  }
